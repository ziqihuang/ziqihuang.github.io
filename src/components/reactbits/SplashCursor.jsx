...
// WebGL fluid-simulation cursor (React Bits). Extended with COLOR + RAINBOW_MODE
// so the splat color can follow an external theme color when rainbow is off.
function SplashCursor({
  ...
  const rainbowRef = useRef(RAINBOW_MODE);
  ...
  useEffect(() => {
    rainbowRef.current = RAINBOW_MODE;
  }, [RAINBOW_MODE]);
  ...
  useEffect(() => {
    ...
    function getWebGLContext(c) {
      ...
      let glCtx = c.getContext("webgl2", params);
      const isWebGL2 = !!glCtx;
      if (!isWebGL2) {
        glCtx =
          c.getContext("webgl", params) || c.getContext("experimental-webgl", params);
      }
      let halfFloat;
    ...
    function generateColor() {
      if (!rainbowRef.current && colorRef.current) {
        const c = hexToNormalizedRgb(colorRef.current);
